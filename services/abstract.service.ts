...
const listData = async (Model: any, list = -1, projections: object = {}) => {
  try {
    let results = [];
    if (list > -1) {
...
const getByConditions = async (
  ...
  try {
    let result = [];
    if (populate.path) {
      result = await Model.find(conditions, projections)
        .populate(populate)
        .lean();
    } else {
...
export class Abstract implements CRUD {
  ...
  async list(list = -1, _page = -1, projections: object = {}) {
    return listData(this.Model, list, projections);
  }
